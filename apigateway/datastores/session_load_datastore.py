...
class SessionLoadDatastore(object):
    def __init__(self, mongo_collection='sessionload'):
        self.mongo_collection = mongo_collection

    @xray_recorder.capture('datastore.SessionLoadDatastore.get')
    ...
    @xray_recorder.capture('datastore.SessionLoadDatastore.put')
    def put(self, items):
    ...
    @xray_recorder.capture('datastore.SessionLoadDatastore._query_mongodb')
    def _query_mongodb(self, session_id, user_id, event_date_time, start_date_time, end_date_time):
        ...
        if session_id is not None:
            ...
            if mongo_result is not None:
                session = SessionLoad.json_deserialise(mongo_result)
                return session
            else:
            ...
            if user_id is None:
                ...
                if event_date_time is not None:
                    ...
                    query['event_date_time'] = {'$regex': f'^{event_date}'}
                elif start_date_time is not None and end_date_time is not None:
                    query['event_date_time'] = {'$gte': format_datetime(start_date_time), '$lte': format_datetime(end_date_time)}
                else:
                ...
                for session in mongo_cursor:
                    ret.append(SessionLoad.json_deserialise(session))
                return ret

    @xray_recorder.capture('datastore.SessionLoadDatastore._put_mongodb')
