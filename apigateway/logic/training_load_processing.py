...
class TrainingLoadProcessing(object):
    ...
    def set_high_relative_load_sessions(self, user_stats, training_sessions):
        ...
        average_5_day_tissue_load_list = [f.tissue_load for f in self.last_5_days_training_sessions]
        ...
        average_20_day_tissue_load_list = [f.tissue_load for f in self.last_20_days_training_sessions]
        ...
        average_5_day_power_load_list = [f.power_load for f in self.last_5_days_training_sessions]
        ...
        average_20_day_power_load_list = [f.power_load for f in self.last_20_days_training_sessions]
        self.average_power_load_20_day = self.get_average_for_error_ranges(average_20_day_power_load_list,
                                                                            0.95 * (self.expected_weekly_workouts * 3))  # adjusted expected weekly workouts by 20/21 of value

        tissue_load_5_20_lowest_value = self.tissue_load_5_20().lowest_value()
        power_load_5_20_lowest_value = self.power_load_5_20().lowest_value()

        tissue_load_percent = 50
        power_load_percent = 50

        if tissue_load_5_20_lowest_value is not None and tissue_load_5_20_lowest_value > 1.2:
            tissue_load_percent = min(100, ((tissue_load_5_20_lowest_value - 1.0) * 100) + 50)

        if power_load_5_20_lowest_value is not None and power_load_5_20_lowest_value > 1.2:
            power_load_percent = min(100, ((power_load_5_20_lowest_value - 1.0) * 100) + 50)
    ...
    def get_average_for_error_ranges(self, error_range_list, expected_weekly_workouts):
        ...
        if len(lower_adjustments) == len(upper_adjustments) and len(lower_adjustments) == len(error_range_list): # just use observed values
        ...
        if len(lower_bound_list) > 0:
            ...
            average_range.lower_bound = min(average_range.lower_bound, lower_bound_value_range.lower_bound)
