            ...
            if total_load is not None and total_load.lowest_value() > 0:  # TODO - this might be temporary and not a detailed enough condition (it's essentially any concentric or eccentric load)
                ...
                if adaptation_type == AdaptationType.power_drill or adaptation_type == AdaptationType.power_explosive_action:
                    ...
                    if ((movement_action.speed == MovementSpeed.fast or movement_action.speed == MovementSpeed.explosive) and
                        ...
                        self.session_detailed_load.add_load(DetailedAdaptationType.speed,
                                                            total_load)
                        self.add_muscle_load(muscle, DetailedAdaptationType.speed, total_load)
                    ...
                    if (duration is not None and duration >= 45 and
                        ...
                        self.session_detailed_load.add_load(DetailedAdaptationType.sustained_power,
                                                            training_load_range)
                        self.add_muscle_load(muscle, DetailedAdaptationType.sustained_power, total_load)

                    # power
                    if ((movement_action.speed == MovementSpeed.fast or movement_action.speed == MovementSpeed.explosive) and
                            movement_action.resistance == MovementResistance.mod):
                        self.session_detailed_load.add_load(DetailedAdaptationType.power, total_load)
                        self.add_muscle_load(muscle, DetailedAdaptationType.power, total_load)
                ...
                if adaptation_type == AdaptationType.power_explosive_action and rpe is not None:
                    ...
                    if 3 <= rpe <= 4.5 and (movement_action.speed == MovementSpeed.fast or movement_action.speed == MovementSpeed.explosive):
                        self.session_detailed_load.add_load(DetailedAdaptationType.maximal_power, total_load)
                        self.add_muscle_load(muscle, DetailedAdaptationType.maximal_power, total_load)
        ...
        if adaptation_type == AdaptationType.strength_endurance_cardiorespiratory:
            if percent_max_hr is not None:
                ...
                if 86 <= percent_max_hr:
                    self.session_detailed_load.add_load(DetailedAdaptationType.anaerobic_interval_training,
                                                        training_load_range)
