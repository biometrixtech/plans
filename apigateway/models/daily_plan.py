...
class DailyPlan(Serialisable):
    ...
    def define_landing_screen(self):
        if not self.daily_readiness_survey_completed():
            return 0.0, 0.0
        elif self.post_recovery is not None and self.post_recovery.impact_score < 1.5:
            self.display_exercises = True
            return 2.0, 1.0
            ...
            return 2.0, None
        elif self.pre_recovery is not None and self.pre_recovery.impact_score < 1.5:
            self.display_exercises = True
            return 0.0, 1.0
