...
def handle_session_create():
    ...
    if 'post_session_survey' in request.json:
        ...
        athlete_stats.post_session_soreness = [s for s in athlete_stats.post_session_soreness if s.reported_date_time is not None and (current_time - s.reported_date_time).total_seconds() < 172800]
        athlete_stats.post_session_pain.extend(severe_pain)
        athlete_stats.post_session_pain = [s for s in athlete_stats.post_session_pain if s.reported_date_time is not None and (current_time - s.reported_date_time).total_seconds() < 172800]
