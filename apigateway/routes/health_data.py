...
def handle_previous_health_data_write():
    ...
    if 'sessions' in request.json and len(request.json['sessions']) > 0:
        all_plans, all_session_heart_rates = SurveyProcessing().process_historic_health_data(user_id, request.json['sessions'], all_plans, event_date)
    else:
        all_plans = []
        all_session_heart_rates = []
    
    if "sleep_data" in request.json and len(request.json['sleep_data']) > 0:
        all_sleep_history = SurveyProcessing().process_historic_sleep_data(user_id, request.json['sleep_data'])
    else:
        all_sleep_history = []
