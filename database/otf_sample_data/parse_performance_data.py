...
def get_rowing_floor_sections(user):
    rower_data = pd.read_csv(f'performance_data_06_08/Anonymous{user}_Rower_Detailed.csv')
    sort_data(rower_data)
    timezone = get_timezone(rower_data.datecreated[0])
...
def get_treadmill_section(user):
    ...
    treadmill_data['block_speed'] = get_block(treadmill_data.speed_mph, min_change=.15)
    del treadmill_data['speed_mph']
    treadmill_data['block_incline'] = get_block(treadmill_data.incline, min_change=1)
    treadmill_data['speed_incline'] = treadmill_data.block_speed + treadmill_data.block_incline
    treadmill_data['block'] = get_block(treadmill_data.speed_incline, min_change=1)
...
def get_completed_workout(user, planned_workout):
    treadmill_sections = get_treadmill_section(user)
    rower_floor_sections = get_rowing_floor_sections(user)
    ...
    for section in planned_workout['sections']:
        if not started:
            if 'row' in section['name']:
                non_rowing_exercises = extract_ex_from_row_section(section)
